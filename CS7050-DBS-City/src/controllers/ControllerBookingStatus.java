...
public class ControllerBookingStatus extends Thread {
	...
	public ControllerBookingStatus(CityUI userInterface) {
    	...
    	try {
	    	...
	    	if (bookingReference != null && !bookingReference.isEmpty()) {
	    		...
	    		if (result == 1) {
		    		...
		    		if (response == null) {
		    			ui.writeNotification("A problem occurred sending check request");
		    		}
		    		else if (response.getStatus() == Response.ERROR){
		            	ui.writeNotification("A problem occurred checking booking state in the server");
		        	}
